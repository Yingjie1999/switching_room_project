...
def img42_1(file_path):
    ...
    lower_hsv1 = np.array([0, 43, 100])  # 提取颜色的低值 yellow and red [26, 43, 46]
    high_hsv1 = np.array([34, 255, 255])  # 提取颜色的高值 [34, 255, 255]
    lower_hsv2 = np.array([156, 43, 46]) # 提取颜色的低值 red [156, 43, 46]
    high_hsv2 = np.array([180, 255, 255]) # 提取颜色的高值 [180, 255, 255]
    ...
    h, w = mask1.shape[:2]
    obj = img_obj1[int(0.3*h):int(0.7*h), int(0.1*w):int(0.9*w)] # mask1
    img_objs.append(obj)
    cv2_show('4', obj)
    return img_objs

def Point_fourty_two(file_name1):
    Point_info = []
    results = {}
    result_imgs = img42_1(file_name1)
...
if __name__ == '__main__':
    img1_path = './images3/42-1.JPG'
    result_imgs = img42_1(img1_path)
    for i in range(len(result_imgs)):
        cv2_show('obj{}'.format(i), result_imgs[i])
    # results['变压器温度A'] = dispatchNum_recog(result_imgs[1])
    # results['调度号'] = dispatchNum_recog(result_imgs[0])
