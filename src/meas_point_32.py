...
def img32_2(file_path):
    ...
    mask1 = cv2.bitwise_or(mask_1, mask_2)
    #cv2_show#('5', mask1)
    h = int(mask1.shape[0] / 3 - 1)
    for i in range(3):
        char = mask1[i*h:(i+1)*h, 0:mask1.shape[1]]
        print(char.shape)
...
def Point_thirty_two(file_name32_1, file_name32_2):
    # Point_info = []
    # results = {}
    # result_imgs = img32_2(file_name32_1)
...
#     img1_path = './images2/32-2.JPG'
#     results = {}
#     result_imgs = img32_2(img1_path)
#     for i in range(len(result_imgs)):
#         #cv2_show#('obj{}'.format(i), result_imgs[i])
#         # results['电压1'] = dispatchNum_recog(result_imgs[i])
#     cv2.waitKey(0)
#     cv2.destroyAllWindows()
