...
def lianpian_split(img):
    ...
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
        # 一个筛选，可能需要看识别条件而定，有待优化
        if w / h < 5 and w / h > 1 and w < 0.8 * split_w and h < 0.8 * split_h and h > 0.1 * split_h and x > 10:
            boundRect.append([x, y, w, h])
            # 画一个方形标注一下，看看圈的范围是否正确
            # red_dil = cv2.rectangle(image_split, (x, y), (x + w, y + h), 255, 2)
    ...
    lianpian_area = [black_area[0]-black_area[2]//2,black_area[1]-black_area[3]*1.5,black_area[2]*2.5,black_area[3]*1.25]
...
def lianpian_recog(img):
    ...
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
        # 一个筛选，可能需要看识别条件而定，有待优化
        if  h > img.shape[0]//3 and h < img.shape[0] * 0.95 and w > img.shape[1]//15:
            if w/h > 1.6:
...
def b_w_key_split2(img, lianpian_location):
    img = img
    h,w = gain_h_and_w(img)
...
def b_w_key_recog2(img):
    ...
    # #cv2.imshow#("gray", gray)
    ret, image_er = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)  # OTSU最大类间方差法
    circles = cv2.HoughCircles(image_er, cv2.HOUGH_GRADIENT, 1, 200, param1=200, param2=35, minRadius=20,
                               maxRadius=image_er.shape[0] //2)
    # circles = np.array([])
    if circles is not None:
        circles = np.uint16(np.around(circles))  # around对数据四舍五入，为整数
    ...
    for i in range(len(cir)):
        ...
        edge = cv2.Canny(image_yuan, 50, 150)
        #cv2.imshow#("bound", edge)
        lines = cv2.HoughLinesP(edge, 1, np.pi / 180, threshold=30, minLineLength=20)
        for x1, y1, x2, y2 in lines[:, 0]:
            cv2.line(image_zhong, (x1, y1), (x2, y2), (0, 0, 255), 2)
        # print("line", lines)
        #cv2.imshow#("xian,", image_zhong)
...
def instruct_led_split(img, bwkey_area):
    img = img
    h,w = gain_h_and_w(img)
...
def instruct_led_recogn(img):
    ...
    circles = cv2.HoughCircles(image_er, cv2.HOUGH_GRADIENT, 1, 200, param1=150, param2=16, minRadius=50,
...
def handcar_split(img, last_location):
    img = img
    h,w = gain_h_and_w(img)
...
def handcar_led_recog(img):
    ...
    # #cv2.imshow#("gray", gray)
    # ret, image_er = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)  # OTSU最大类间方差法
    # contours1, hierarchy = cv2.findContours(image_er, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 找轮廓
    #
    # contours2, hierarchy = cv2.findContours(mask_yellow, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 找轮廓
    # boundRect = []
    # color = []
    # for c in contours1:
    #     x, y, w, h = cv2.boundingRect(c)
    ...
    for i in range(len(boundRect_)):
        img_quar = img[boundRect_[i][1]:boundRect_[i][1]+boundRect_[i][3], boundRect_[i][0]:boundRect_[i][0]+boundRect_[i][2]]
        ...
        if object_H > 156 and object_H < 180:
            color.append("合闸")
        elif object_H >= 0 and object_H < 13:
            color.append("合闸")
        elif object_H > 13 and object_H < 34:
            color.append("分闸")
        else:
            color.append(" ")
    print("color:", color)
...
def running_led_split(img, last_location):
    img = img
    h,w = gain_h_and_w(img)
...
def running_led_recog(img):
    ...
    led_list = []
    for i in range(len(boundRect)):
        img_quar = img[boundRect[i][1]:boundRect[i][1]+boundRect[i][3], boundRect[i][0]:boundRect[i][0]+boundRect[i][2]//2]
        hsv = cv2.cvtColor(img_quar, cv2.COLOR_BGR2HSV)
        H, S, V = cv2.split(hsv)
        # print("HSV", H, S, V)
        v = V.ravel()[np.flatnonzero(V)]  # 亮度非零的值
...
def Point_twelve(file_name1, file_name2):
    ...
    print("point_twelve:", Point_list)
    return tran(Point_list)

# if __name__ == '__main__':
#     file_name = 'E:\\desktop\\images2\\12-2.JPG'
...
#     _, handcar_led_area = thr_handcar_split(file_name, led_area)
