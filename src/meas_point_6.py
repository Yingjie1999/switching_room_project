    ...
    if boundRect is not None and len(boundRect) == 3:
        ...
        for i in range(len(boundRect)):
            ...
            if object_H >= 156 and object_H <= 180:
                color.append("合闸")
            elif object_H >= 0 and object_H <= 12:
                color.append("合闸")
            elif object_H > 12 and object_H <= 34:
                color.append("分闸")
            else:
                color.append(" ")
            print("color:", color)
...
def running_led_recog(img):
    ...
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
        # 一个筛选，可能需要看识别条件而定，有待优化
        if  h < img.shape[0] * 0.95 and w/h >3 and w/h<6 and w > img.shape[1]//5 :
            boundRect.append([x, y, w, h])
            # 画一个方形标注一下，看看圈的范围是否正确
            red_dil = cv2.rectangle(img, (x, y), (x + w, y + h), 255, 2)
...
def Point_six(file_name1, file_name2):
