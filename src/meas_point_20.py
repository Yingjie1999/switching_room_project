...
def lianpian_split(img):
    ...
    lianpian_area = [black_area[0]-black_area[2]//2,black_area[1]-black_area[3]*1.55,black_area[2]*2,black_area[3]*1.25]
...
def b_w_key_recog2(img):
    ...
    # #cv2.imshow#("cvcanny", canny)
    ret, image_er = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)  # OTSU最大类间方差法
    circles = cv2.HoughCircles(image_er, cv2.HOUGH_GRADIENT, 1, 200, param1=200, param2=40, minRadius=20,
                               maxRadius=image_er.shape[0] //2)
    # circles = np.array([])
    print(circles)
    if circles is not None:
...
def instruct_led_recogn(img):
    ...
    circles = cv2.HoughCircles(image_er, cv2.HOUGH_GRADIENT, 1, 200, param1=150, param2=25, minRadius=50,
...
def handcar_split(image, last_location):
    img = image
    h,w = gain_h_and_w(img)
    handcar_location =[last_location[0],last_location[1]-last_location[3]//3*2, last_location[2], last_location[3]]
...
def handcar_led_recog(img):
    ...
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
        # 一个筛选，可能需要看识别条件而定，有待优化
        if  h < img.shape[0] * 0.95 and w/h >3 and w>img.shape[1]//8 and w / h <6:
            boundRect.append([x, y, w, h])
            # 画一个方形标注一下，看看圈的范围是否正确
            red_dil = cv2.rectangle(img, (x, y), (x + w, y + h), 255, 2)
    ...
    if boundRect is not None and len(boundRect) == 3:
        ...
        print(boundRect)
        for i in range(len(boundRect)):
            img_area = [boundRect[i][0] + boundRect[i][3]//3*2, boundRect[i][1] - boundRect[i][3]*3, boundRect[i][2]-boundRect[i][3], boundRect[i][3]*2]
...
def running_led_split(image, last_location):
    img = image
    h,w = gain_h_and_w(img)
    running_location =[last_location[0] - last_location[3]//3*4, last_location[1], last_location[3], last_location[3]*2]
...
# def Point_twenty(file_name1, file_name2):
...
def Point_twenty(img_path, code_info):
