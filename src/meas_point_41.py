...
def img41_2(file_path):
    ...
    if circles is None:
        print('储能指示亮')
        results["储能指示"] = '亮'
        lower_hsv1 = np.array([11, 100, 140])  # 提取颜色的低值 yellow [26, 43, 46]
    ...
    if o is not None:
        ...
        raise ValueError
    box = np.array([min(X_obj[:, 1])+box3[0], min(X_obj[:, 0])+box3[1], max(X_obj[:, 1])+box3[0], max(X_obj[:, 0])+box3[1]]).astype(np.int) # xmin, ymin, xmax, ymax
    print(box)
    box = (box / f_xy).astype(np.int)
...
def Point_fourty_one(file_name1, file_name2):
    # Point_info = [ '合闸指示', '储能指示', '分闸指示', '合闸按钮', '分闸按钮',]
    # result_imgs = img41_1(file_name1)
    # #cv2_show#('result', result_imgs)
    # results2 = img41_2(file_name2)
    # # results2['电压1'] = str(0)
    # # results2['电压2'] = str(0)
    # # results2['电压3'] = str(0)
    # # results2['电压4'] = str(0)
...
if __name__ == '__main__':
    img1_path = './images3/41-1.JPG'
    img2_path = './images3/41-2.JPG'
    result = img41_1(img1_path)
    #cv2_show#('result', result)
    results2 = img41_2(img2_path)
    results2['电压1'] = str(0)
    results2['电压2'] = str(0)
    results2['电压3'] = str(0)
    results2['电压4'] = str(0)
