...
def instruct_led_recogn(img):
    ...
    # #cv2.imshow#("gray", gray)
    canny = cv2.Canny(gray, 50, 150)
    #cv2.imshow#("inst_canny",canny)
    ret, image_er = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)  # OTSU最大类间方差法
    #cv2.imshow#("led_er",image_er)
    circles = cv2.HoughCircles(canny, cv2.HOUGH_GRADIENT, 1, 200, param1=150, param2=30, minRadius=30,
...
# def Point_twenty_two(file_name1, file_name2):
...
def Point_twenty_two(img_path, code_info):
