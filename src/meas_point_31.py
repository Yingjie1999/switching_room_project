...
def img31_2(file_path):
    img = cv2.imread(file_path)
    h, w = img.shape[:2]
    box = np.array([0.25*w, 0, 0.75*w, 0.5*h]).astype(np.int) # xmin, ymin, xmax, ymax
    img = img[box[1]:box[3], box[0]:box[2]]
    cv2_show('img', img)
    f_xy = 0.2
    img1 = cv2.resize(img.copy(), None, fx=f_xy, fy=f_xy)
...
def Point_thirty_one(file_name1, file_name2):
    Point_info = []
    results1 = img31_1(file_name1)
    # result_imgs = img31_2(img1_path)
...
if __name__ == '__main__':
    img1_path = './images4/33-1.JPG' # Need to modify!!
    img2_path = './images4/33-2.JPG' # Need to modify!!
    results1 = img31_1(img1_path)
    result_imgs = img31_2(img2_path)
    cv2_show('result', result_imgs)
    print(results1)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
