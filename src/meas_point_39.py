...
def img39_2(file_path):
    ...
    # 合闸按钮
    box1 = np.array([x-21*r, y-3*r, x-13*r, y+3*r]).astype(np.int)
    ...
    # 合闸指示
    box1 = np.array([x - 21 * r, y - 10 * r, x - 13 * r, y - 4 * r]).astype(np.int)
    obj = img1[box1[1]:box1[3], box1[0]:box1[2]]
    cv2_show('3', obj)
    hsv = cv2.cvtColor(obj, cv2.COLOR_BGR2HSV) # 色彩空间转换为hsv，便于分离
    ...
    # 储能指示
    box1 = np.array([x-13*r, y-10*r, x-5*r, y-4*r]).astype(np.int)
    obj = img1[box1[1]:box1[3], box1[0]:box1[2]]
    cv2_show('3', obj)
    hsv = cv2.cvtColor(obj, cv2.COLOR_BGR2HSV) # 色彩空间转换为hsv，便于分离
    lower_hsv1 = np.array([26, 43, 46])  # 提取颜色的低值 yellow [26, 43, 46] 11
    high_hsv1 = np.array([34, 100, 180])  # 提取颜色的高值 [34, 255, 255]
    mask1 = cv2.inRange(hsv, lowerb=lower_hsv1, upperb=high_hsv1)
    mask=cv2.Canny(mask1, 30, 100)
    cv2_show('m2', mask)
    circles = cv2.HoughCircles(mask,cv2.HOUGH_GRADIENT,1,20,
    ...
    hsv = cv2.cvtColor(obj, cv2.COLOR_BGR2HSV) # 色彩空间转换为hsv，便于分离
    lower_hsv1 = np.array([35, 100, 46]) # 提取颜色的低值 green [35, 43, 46]
    high_hsv1 = np.array([100, 255, 180]) # 提取颜色的高值 [77, 255, 255]
    ...
    # 对box3检测
    box1 = np.array([x-14*r, y-20*r, x-4*r, y-11*r]).astype(np.int)
    obj = img1[box1[1]:box1[3], box1[0]:box1[2]]
    cv2_show('3', obj)
...
def Point_thirty_nine(file_name1, file_name2):
    Point_info = [ '合闸指示', '储能指示', '分闸指示', '合闸按钮', '分闸按钮']
    # result_imgs = img39_1(file_name1)
    # cv2_show('result', result_imgs)
    results2 = img39_2(file_name1)
    # results2['电压1'] = str(0)
    # results2['电压2'] = str(0)
    # results2['电压3'] = str(0)
    # results2['电压4'] = str(0)
    ...
    print(results2)
    Point_list = list(results2.values())
    print(Point_list)
    return Point_list

if __name__ == '__main__':
    img1_path = './images3/41-1.JPG' # Need to modify!!
    img2_path = './images3/41-2.JPG' # Need to modify!!
    # result = img41_1(img1_path)
    # cv2_show('result', result)
    results2 = img39_2(img2_path)
    results2['电压1'] = str(0)
    results2['电压2'] = str(0)
    results2['电压3'] = str(0)
    results2['电压4'] = str(0)
