...
def L_led_recog(image):
    ...
    circles = cv2.HoughCircles(canny, cv2.HOUGH_GRADIENT, 1, 35, param1=200, param2=30, minRadius=20,
...
def lianpian_recog(img):
    ...
    #cv2.imshow#("mask_red", mask_red)
    #cv2.imshow#("mask",mask)
    contours1, hierarchy1 = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 找轮廓
    # cv2.drawContours(img, contours, -1, (0, 255, 0), 3)  # 把边缘给画出来
    # #cv2.imshow#("bound",img)
    boundRect = []
    for c in contours1:
        x, y, w, h = cv2.boundingRect(c)
        # 一个筛选，可能需要看识别条件而定，有待优化
        if  h < img.shape[0] * 0.95 and w / h <2 and w/h>0.1 and h >img.shape[0]//3 and w>img.shape[1]//15:
            boundRect.append([x, y, w, h])
            # 画一个方形标注一下，看看圈的范围是否正确
            red_dil = cv2.rectangle(img, (x, y), (x + w, y + h), 255, 2)
            #cv2.imshow#("red",red_dil)
...
def instruct_led_recogn(img):
    ...
    circles = cv2.HoughCircles(canny, cv2.HOUGH_GRADIENT, 1, 200, param1=150, param2=50, minRadius=50,
                               maxRadius=image_er.shape[0] //4)
    # circles = np.array([])
    if circles is not None:
        circles = np.uint16(np.around(circles))  # around对数据四舍五入，为整数
...
def running_led_split(image, last_location):
    img = image
    h,w = gain_h_and_w(img)
    running_location =[last_location[0] - last_location[3]//10*11, last_location[1], last_location[3], last_location[3]*2]
...
def Point_eleven(file_name1, file_name2):
    img1 = cv2.imread(file_name1)
    img2 = cv2.imread(file_name2)
    Point_list = ['灭', '灭', '灭', '就地', '预合分后', '分', '分', '开', '开', '灭', '灭', '灭', ' ', ' ', ' ', '灭', '灭', '灭', '灭', '灭', '灭']
    ...
    # Point_list.append(handcar_led_recog(handcar_img))                 #黄色阈值不一样 这里是16可以用
...
#     maxindex = np.argmax(boundRect,axis=0)
#     black_area = boundRect[maxindex[2]]
#     lianpian_area = [black_area[0]-black_area[2]//2,black_area[1]-black_area[3]*1.8,black_area[2]*2,black_area[3]*1.25]
#     lianpian_area = [int (i) for i in lianpian_area]
#     print("lianpian",lianpian_area)
#     lianpian_image = image_split[lianpian_area[1]:lianpian_area[1]+lianpian_area[3],lianpian_area[0]:lianpian_area[0]+lianpian_area[2]]
#     #cv2.imshow#("img",lianpian_image)
...
#     ret, image_er = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)  # OTSU最大类间方差法
#     #cv2.imshow#("led_er",image_er)
#     circles = cv2.HoughCircles(image_er, cv2.HOUGH_GRADIENT, 1, 200, param1=150, param2=20, minRadius=50,
#                                maxRadius=image_er.shape[0] //4)
#     # circles = np.array([])
#     if circles is not None:
#         circles = np.uint16(np.around(circles))  # around对数据四舍五入，为整数
...
#     img = image
#     h,w = gain_h_and_w(img)
#     handcar_location =[last_location[0],last_location[1]-last_location[3]//4*3, last_location[2], last_location[3]//4*3]
#     print("handcar_",handcar_location)
...
#             elif object_H > 0 and object_H < 10:
#                 color.append("合闸")
#             elif object_H > 25 and object_H < 34:
#                 color.append("分闸")
#             else:
#                 color.append(" ")
#             print("color:", color)
...
#     img = image
#     h,w = gain_h_and_w(img)
#     running_location =[last_location[0] - last_location[2]//2, last_location[1], last_location[2]//2, last_location[3]*2]
#     print("running",running_location)
#     led_img = img[running_location[1]:running_location[1]+running_location[3],running_location[0]:running_location[0]+running_location[2]]
#     #cv2.imshow#("run_led",led_img)
#     return led_img, running_location
...
#     return eval(f"[{list_str}]")
#
# def Point_seven(file_name1, file_name2):
#     img1 = cv2.imread(file_name1)
#     img2 = cv2.imread(file_name2)
#     Point_list = []
#     num_img = dispatchNum_split(img1)
...
#     # digit_image_ = split.detect_and_split_1(digit_img)
#     # Point_list.append(digit_recog(digit_image_))
#
#     print("point_seven:",Point_list)
#     return tran(Point_list)
#
#
# #
